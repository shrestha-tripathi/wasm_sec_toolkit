...
mod tests {
    ...
    fn test_sporkmarmot(data: &[u8], output_suffix: Option<&'static str>) {
        let mut buf = Vec::new();
        let mut f = File::open("./sporkmarmot_riding_bufficorn.png").unwrap();
        ...
        if let Some(output_suffix) = output_suffix {
            let mut f = File::create(format!(
                "./sporkmarmot_riding_bufficorn_steg_{}.png",
                output_suffix
            ))
            .unwrap();

            f.write_all(&steg).unwrap();
        }
    ...
    #[test]
    fn test_hello_world() {
        test_sporkmarmot(b"hello world!", Some("hello_world"));
    }
